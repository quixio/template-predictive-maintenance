...
async def generate_data_and_close_stream_async(topic_producer: qx.TopicProducer, printer: str, initial_delay: int):
    ...
    while True:
        ...
        stream.properties.metadata["failures_str"] = str(
            [str(datetime.fromtimestamp(x / 1000000000).isoformat()) for x in failure_timestamps])
        stream.properties.metadata["failures_replay_speed_str"] = str(
            [str(datetime.fromtimestamp(x / 1000000000).isoformat()) for x in failure_replay_speed_timestamps])

        print(str([str(datetime.fromtimestamp(x / 1000000000).isoformat()) for x in failure_timestamps]))
        print(str([str(datetime.fromtimestamp(x / 1000000000).isoformat()) for x in failure_replay_speed_timestamps]))
